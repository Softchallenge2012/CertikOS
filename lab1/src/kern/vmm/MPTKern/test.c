...
int MPTKern_test1()
{
  ...
  if (get_ptbl_entry_by_va(1, vaddr) != 0) {
    dprintf("test 1 get ptbl entry by va failed.\n");
    // return 1;
  }
  if (get_pdir_entry_by_va(1, vaddr) != 0) {
    dprintf("test 1 get pdir entry by va failed.\n");
    // return 1;
  }
 
  map_page(1, vaddr, 100, 7);
  if (get_ptbl_entry_by_va(1, vaddr) == 0) {
    dprintf("test 1 map get ptbl entry=%d by va failed.\n", get_ptbl_entry_by_va(1, vaddr));
    // return 1;
  }
  if (get_pdir_entry_by_va(1, vaddr) == 0) {
    dprintf("test 1 map  pdir entry=%d by va failed.\n", get_pdir_entry_by_va(1, vaddr));
    // return 1;
  }
