...
int PTQueueInit_test2()
{
  ...
  if (tcb_get_prev(2) != NUM_IDS || tcb_get_next(2) != 3) {
    dprintf("test 2 2 previous = %d next = %d head=%d tail=%d failed.\n", tcb_get_prev(2), tcb_get_next(2), tqueue_get_head(0), tqueue_get_tail(0));
    //  return 1;
  }
  if (tcb_get_prev(3) != 2 || tcb_get_next(3) != 4) {
    dprintf("test 2 3 previous = %d next = %d head =%d tail=%d  failed.\n", tcb_get_prev(3), tcb_get_next(3), tqueue_get_head(0), tqueue_get_tail(0));
    // return 1;
  }
  if (tcb_get_prev(4) != 3 || tcb_get_next(4) != NUM_IDS) {
    dprintf("test 2 4 previous = %d next = %d failed.\n", tcb_get_prev(4), tcb_get_next(4));
    // return 1;
  }
  tqueue_remove(0, 3);
  if (tcb_get_prev(2) != NUM_IDS || tcb_get_next(2) != 4) {
    dprintf("test 2 2 previous=%d next=%d failed.\n", tcb_get_prev(2), tcb_get_next(2));
    // return 1;
  }
  if (tcb_get_prev(3) != NUM_IDS || tcb_get_next(3) != NUM_IDS) {
    dprintf("test 2 3 previous=%d next=%d failed.\n", tcb_get_prev(3), tcb_get_next(3));
    // return 1;
  }
  ...
  if (pid != 2 || tcb_get_prev(pid) != NUM_IDS || tcb_get_next(pid) != NUM_IDS
   || tqueue_get_head(0) != 4 || tqueue_get_tail(0) != 4) {
    dprintf("test 2 2 pid=%d tcb_get_prev(pid)=%d  tcb_get_next(pid)=%d head=%d tail=%d  failed.\n", pid, tcb_get_prev(pid), tcb_get_next(pid), tqueue_get_head(0), tqueue_get_tail(0));
    return 1;
  }
